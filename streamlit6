...
if not df.empty :
  ...
  id_to_delete=st.selectbox("Επέλεξε πιο id θέλεις να διαγράψεις", available_ids)

  if st.button("Διαγραφή επιλεγμένης εργασίας"):
    cursor.execute("DELETE FROM tasks WHERE id = ?", (id_to_delete,))
    conn.commit()
    st.success(f"Το τασκ {id_to_delete} διαγραφτηκε επιτυχώς")
    st.rerun()
